...
Todos leen sus claves de variables de entorno (`.env`), nunca hardcodeadas.
...
cp .env.example .env      # y rellená tus claves
```
...
# 3) Levantar el SOAR handler y mandarle una alerta de ejemplo
...
Ver `docs/06-soar-automation.md` para la config de Wazuh (`ossec.conf`) que envía las alertas.

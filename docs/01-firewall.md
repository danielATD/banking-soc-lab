...
pfSense es el router/firewall central del "Banco Demo S.A.": 5 interfaces, una por zona, con
default-deny entre zonas. Las "VLANs" del diseño se implementan como **redes internas separadas de
VirtualBox** (una red = una zona), no como VLANs 802.1Q: se logra el mismo aislamiento sin un switch
gestionado.
...
virtual se conecta cada cable. Se configura con la VM **apagada**, desde la terminal del host
con `vboxmanage` (o la GUI de VirtualBox, que solo muestra 4 adaptadores; la CLI llega a 8).
...
3. Encender la VM. FreeBSD detecta la placa nueva → aparece `em5`.
...
completo** (Status → Filter Reload muestra el error). Nos pasó exactamente eso: 6 reglas "visibles"
en la GUI y `pfctl -sr` sin ninguna; el banner pendiente en las capturas lo delató.
...
- **E_ACCESSDENIED al crear vboxnet0 con IP 10.40.0.2 (21/07):** VirtualBox restringe los rangos
  host-only por seguridad (default-deny de rangos); el fix es autorizarlo en
